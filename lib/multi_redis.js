...
MultiRedis.prototype._bind = function (client) {
  var self = this;
...
MultiRedis.prototype._bindSub = function (client) {
  var self = this;
  commands.events.forEach(function (evt) {
    client.on(evt, function () {
...
commands.subscriberCmds.forEach(function (command) {
  MultiRedis.prototype[command] = function () {
    //first stop ping
    this._stopPing();
    //save for reconect
    this.subCmds.push([command, arguments]);
 ...
 * @return {[type]}
 */
MultiRedis.prototype._startPing = function() {
  if (this._pingTimer) {
    return false;
  }
  this._ping();
  //not in sub/pub mode, clean subCmds
  this.subCmds = []; 
  var self = this;
...
MultiRedis.prototype._stopPing = function () {
