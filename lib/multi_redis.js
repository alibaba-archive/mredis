      ...
      var cb = function(err) {
        if (err && typeof err === 'string') {
          if (err.indexOf('connect ECONNREFUSED') < 0) {
            ...
            if(self.clients.length===1) {
              return fn.apply(null, new Error('All servers are down.'));
            } else {
      ...
      var cb = function(err) {
        ...
        if (err && typeof err === 'string') {
          if (err.indexOf('connect ECONNREFUSED') < 0) {
            ...
            if(called >= self.clients.length) {
              if(self.clients.length===0 && !haveCalled) {
                haveCalled = true;
                return fn.call(null, new Error('All servers are down.'));
              }
              //if length !== 0, means already do cb before
              if(!haveCalled) {
                haveCalled = true;
        ...
        if (called >= self.clients.length && !haveCalled) {
          haveCalled = true;
          fn.apply(null, cbArg);
        }
      }
